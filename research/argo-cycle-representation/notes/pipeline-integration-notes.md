...
Understanding this in detail matters for two reasons. First, it determines where this pipeline's artifacts would slot into downstream products or analysis layers that consume vertically standardized Argo information. Second, knowing where QC flags, bias corrections, and delayed-mode processing enter the chain clarifies what state the data is in when it reaches the vertical representation step.
