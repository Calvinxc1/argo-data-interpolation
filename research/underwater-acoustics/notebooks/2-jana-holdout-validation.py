...
# %% [markdown]
# ## 1. Load the same cached Bay of Bengal archive used by the Jana replication
#
# The notebook reuses the local cached Argo pull from the replication workflow. If the cached file is unavailable, run `1-jana-study-replication.ipynb` first so the same starting archive is available here.
#
...
if data_file is None:
    raise FileNotFoundError(
        "Could not find a cached Jana replication archive under "
...
#
# The result cache stores both cycle-level diagnostics and the full residual-profile tables so the expensive evaluation pass does not have to be rerun for every plotting change.
#

# %%
results_file = data_path / "jana_holdout_validation.pkl"
override = False
