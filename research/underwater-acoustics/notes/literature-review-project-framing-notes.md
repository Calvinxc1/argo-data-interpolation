...
The clearest literature-backed opening for the current project is a per-profile interpolation study for derived sound speed with explicit comparative validation. Adjacent ocean heat-content work shows that interpolation can materially affect derived ocean quantities, while the draft acoustics source set does not yet show an explicit per-profile interpolation comparison for derived sound-speed structure or strong direct validation of competing profile-processing choices. Lit-review anchor: [Vertical Interpolation Methods Remain Under-Reported](../literature-review.md#vertical-interpolation-methods-remain-under-reported).
...
The draft working estimate for Argo sensor-level sound-speed uncertainty is on the order of 0.04 m/s, with temperature dominating, salinity secondary, and pressure contributing more strongly at depth. This remains a project-note assumption until it is re-derived or checked against full-text error-propagation sources.
...
The operational dependency is also more concrete in the broader source set than in the canonical review alone. The literature review supports the narrower point that Argo-informed operational ocean fields are upstream inputs to acoustically relevant workflows. The stronger HYCOM/NCODA and Navy-workflow translation remains a project-facing interpretation that should stay in notes until it is promoted into the canonical review explicitly. The project therefore reads more cleanly as source-level risk quantification for downstream acoustics than as interpolation methodology for its own sake.
...
A deterministic Jana et al. replication provides a legible baseline. A follow-on uncertainty-aware wrapper around linear, PCHIP, and spline-style profile interpolation can then be evaluated against that baseline through explicit comparative validation, rather than descriptive reconstruction alone, to quantify how much downstream variability in sound speed and acoustically relevant diagnostics is attributable not to the seawater equation or the observing platform, but to the profile-processing decision that the literature often leaves implicit.

The project-facing implication extracted from the literature-review conclusion is procedural rather than bibliographic: any uncertainty-aware interpolation pipeline should make profile-processing decisions explicit, compare multiple profile-level schemes on common Argo cycles, distinguish descriptive reconstruction from direct comparative validation, and separate interpolation-driven spread from sensor and equation uncertainty as explicitly as the current literature allows.
